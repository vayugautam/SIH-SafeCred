...
export default function Apply() {
  ...
  // helper to get user mobile from auth context
  ...
  useEffect(() => {
    const loadIncomeBarrier = async () => {
      try {
        const resp = await axios.get('/api/ml-metadata', { validateStatus: () => true })
  ...
  const verifyIncome = async (mobile: string | undefined, declaredIncome: number) => {
    try {
      ...
      const resp = await axios.post('/api/nbcfdc/verify-income', { mobile }, { validateStatus: () => true })
  ...
  const reportIncomeMismatch = async (payload: { userId?: string; mobile?: string; declaredIncome: number; realIncome?: number; diffPct?: number }) => {
    try {
      await axios.post('/api/report/income-mismatch', payload)
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post('/api/applications', payload, { validateStatus: () => true })
      const data = response.data

      if (response.status === 401) {
        setIsLoading(false)
        navigate('/login?callbackUrl=/apply')
        return
      }
